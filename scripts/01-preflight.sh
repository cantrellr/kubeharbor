...
# openssl pkey handles RSA, ECDSA, and Ed25519 keys; openssl rsa would fail for non-RSA keys.
openssl pkey -in "${BUNDLE_DIR}/${HARBOR_LEAF_KEY_SOURCE}" -noout >/dev/null 2>&1 || fail "Leaf private key failed openssl validation."
